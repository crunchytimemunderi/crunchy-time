...
const defaultCategories = ['chicken', 'oil', 'masala', 'gas', 'packaging', 'other'];
...
function InventoryContent() {
  ...
  const fetchInventory = useCallback(async () => {
    try {
      ...
      // Merge with defaults
      setCategories(Array.from(new Set([...defaultCategories, ...uniqueCategories])));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.item_name || !formData.unit_price || !formData.stock_quantity) {
    ...
    if (!editingItem && items.some(item => item.item_name.toLowerCase() === formData.item_name.toLowerCase())) {
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md">
          ...
          <p className="mt-2 text-sm text-gray-600">Your role: {userData?.role || 'Not set'}</p>
          <Link href="/dashboard" className="mt-4 inline-block text-red-600 hover:underline font-medium">
            ← Back to Dashboard
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {items.filter(item => item.stock_quantity < 10).length > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg mb-6">
            <h3 className="text-yellow-800 font-bold mb-2">⚠️ Low Stock Alert</h3>
            <p className="text-yellow-700 text-sm">
              {items.filter(item => item.stock_quantity < 10).length} item(s) running low on stock (less than 10 units)
            </p>
          </div>
        )}
              ...
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-gray-900">{formatINR(item.unit_price)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-gray-900">
                      <span className={item.stock_quantity < 10 ? 'text-red-600 font-bold' : ''}>
                        {item.stock_quantity}
                      </span>
                    </td>
